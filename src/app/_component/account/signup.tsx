...
export default function SignupForm() {
  ...
  const handleSubmit = useCallback((e: React.FormEvent) => {
    e.preventDefault();
    axios.post("http://localhost:8080/auth/register", userData).then(
      () => {
        loginUser();
        router.push('/');
      }
  ...
  return (
    <form onSubmit={handleSubmit} className="w-[400px] h-[500px] grid content-between">
      <div>
        <div className="static text-6xl text-[#34374d] flex flex-col justify-center">
          Join Meteor
          <span className="text-xl mt-4">Elevate your video calling experience.</span>
        </div>
      </div>
      <div>

        <p className={`top-[-20px] ml-[10px] text-[16px] text-[#ff6468] z-10 ${invalidCredentials ? 'visible animate-shake animate-thrice animate-duration-200 animate-ease-linear' : 'hidden'}`}>User Already Exists!</p>
